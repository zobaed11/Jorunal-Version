...
/*        try {
			...
			consolidateQuery_newResearchCheck();
		} catch (IOException e) {
    ...
    //Put all of the weights and query stuff into one map.
    private void consolidateQuery_newResearchCheck() throws IOException {
        //First go through the query weights
    	String filePath = "/home/C00300901/Desktop/weighted_query.txt";
        ...
        while ((line = reader.readLine()) != null)
        {
            ...
            {
                String key = parts[0];
                Float value = (float) Double.parseDouble(parts[1]);
                allWeights.put(key, value);
            } else {
                System.out.println("ignoring line: " + line);
            }
        }

        for (String key : allWeights.keySet())
        {
            System.out.println(key + ":" + allWeights.get(key));
        }
        reader.close();
    ...
    public void rankAbstracts() {
        ...
        String[] myCount= originalQuery.split(" ");
        ...
        for (int i = 0; i < Config.numSearchedAbstracts && i < abstracts.size(); i++) {
          //for (int i = 0; i < myCount.length && i < abstracts.size(); i++) { 
