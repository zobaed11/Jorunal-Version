...
/*            if (term == "common format" )
            {
            	System.out.println("mamams");
            	System.exit(0);
            }*/
        }
    ...
    private Float computeDistance(Centroid centroid, String term) {
        ...
        ArrayList <String> postiveFiles= new ArrayList<>(files.keySet());
        postiveFiles.retainAll(centroid.files.keySet());
        
        ArrayList <String> negativeFilesTermToCenter= new ArrayList<>(files.keySet());
        negativeFilesTermToCenter.removeAll(centroid.files.keySet());
        
        ArrayList <String> negativeFilesCenterToTerm= new ArrayList<>(centroid.files.keySet());
        negativeFilesCenterToTerm.removeAll(files.keySet());
...
/*        for (String file : files.keySet()) {
        ...
        float ratioOfFileForTerm=0; // c(f, I[T]) / |I[T]|
        float ratioOfFileForBoth=0;
        for (String file : files.keySet()) {
            
        	ratioOfFileForTerm = (float)files.get(file) / (float)termCount; //Gets the count of the term in this file divided by the total number of times that term appeared.
        }
        
        float ratioOfFileForBothPositive=0;
        float ratioOfFileForBothNegativeTermToCenter=0;
        float ratioOfFileForBothNegativeCenterToTerm=0;
        
        for (String file : files.keySet()) {
        	
        	if (postiveFiles.contains(file))
        	{
        		ratioOfFileForBothPositive += ((float)files.get(file) + (float)centroid.files.getOrDefault(file, 0)) / ((float)termCount + (float)centroidCount);
        	}
        	
        	else
        	{
        		ratioOfFileForBothNegativeTermToCenter += (float)files.get(file)  /  (float)termCount  ;
        	}
        }
        ...
        for (String file: centroid.files.keySet()) {
        	if (! postiveFiles.contains(file))
        	{
        		ratioOfFileForBothNegativeCenterToTerm += (float)centroid.files.get(file)  / (float)centroidCount  ;                                                                                                            ;
        	}
        }
        
            
         ratioOfFileForBoth = ratioOfFileForBothPositive-ratioOfFileForBothNegativeTermToCenter-ratioOfFileForBothNegativeCenterToTerm;
          ...
          //  distance = (float) (ratioOfFileForTerm * logPart);
           distance = (float) (ratioOfFileForTerm * ratioOfFileForBoth);
        ...
        //System.out.println(term + " " + centroid.term + " " + distance );
        return -distance;
    }
}
